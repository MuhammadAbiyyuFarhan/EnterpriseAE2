...
<head>
	...
	<?php 
	$koneksi = new mysqli ("localhost","root","","ppc3");
	$quiz = $koneksi->query("SELECT * FROM quiz_list where id =".$_GET['id'])->fetch_array();
	$hist = $koneksi->query("SELECT * FROM history where quiz_id =".$_GET['id'])->fetch_array();
	?>
	...
	<div class="container-fluid admin">
		...
		<div class="card">
			<div class="card-body">
					...
					<?php
					$question = $koneksi->query("SELECT * FROM questions where qid = '".$quiz['id']."' order by id desc ");
					$i = 1 ;
					while($row =$question->fetch_assoc()){
						$opt = $koneksi->query("SELECT * FROM question_opt where question_id = '".$row['id']."' order by RAND() ");
					$answer = $koneksi->query("SELECT * FROM answers where quiz_id ='".$quiz['id']."' and user_id= '".$_SESSION['login_id']."' and question_id = '".$row['id']."'  ")->fetch_array();
					?>
