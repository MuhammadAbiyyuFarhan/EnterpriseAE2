<head>
	...
	$quiz = $koneksi->query("SELECT * FROM quiz_list where id =".$_GET['id']." order by RAND()")->fetch_array();
	?>
		...
		<div class="card">
			<div class="card-body">
				<form action="" id="answer-sheet">
					...
					<?php
					$question = $koneksi->query("SELECT * FROM questions where qid = '".$quiz['id']."' order by order_by asc ");
					$i = 1 ;
...
<script>
	$(document).ready(function(){
		...
		$('#answer-sheet').submit(function(e){
			...
			$.ajax({
				url:'pengguna/submit_answer.php',
				method:'POST',
				data:$(this).serialize(),
